...
class SettingsHelper
{
    ...
    public static function hasSettings(string $settingsKey): bool
    {
        ...
        try {
            $settings = Cache::remember($key, now()->addDay(), function () use ($settingsKey)
            {
    ...
    public static function upsertSettings(string $key, string $value): SettingsProperty
    {
        ...
        try {
            $settingProperty = SettingsProperty::updateOrCreate(['settings_key' => $key], ['settings_value' => $value]);
